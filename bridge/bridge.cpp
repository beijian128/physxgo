...
public:
    ...
    void onContact(const PxContactPairHeader& pairHeader, const PxContactPair* pairs, PxU32 nbPairs) override {
        ...
        for (PxU32 i = 0; i < nbPairs; i++) {
            ...
            cps[i].actors[0] = (uint64_t)(uintptr_t)pairs[i].actors[0];
            cps[i].actors[1] = (uint64_t)(uintptr_t)pairs[i].actors[1];
            ...
            if (nbContacts > 0) {
                ...
                cps[i].impulse[0]     = pt.impulse.get(0);
                cps[i].impulse[1]     = pt.impulse.get(1);
...
int physx_joint_get_break_force(PxJointHandle joint, float* force, float* torque) {
    if (!joint || !joint->joint) return PHYSX_ERROR_NULL_PTR;
    if (force) *force = joint->joint->getBreakForce();
    if (torque) *torque = joint->joint->getBreakTorque();
    return PHYSX_SUCCESS;
}
...
int physx_prismatic_joint_set_limit(PxJointHandle joint, float lower, float upper, float stiffness, float damping) {
    ...
    p->setLimit(PxJointLinearLimitPair(physics::PxTolerancesScale(), lower, upper, PxSpring(stiffness, damping)));
    return PHYSX_SUCCESS;
}
